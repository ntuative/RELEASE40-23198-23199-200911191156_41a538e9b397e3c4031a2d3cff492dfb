package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_402 >= var_298.length)
         {
            return "";
         }
         var petColor:IPetColor = var_298[var_402] as IPetColor;
         var color:uint = petColor.rgb;
         var p:String = _name + String.fromCharCode(10) + addZeroPadding(String(var_1464),3) + String.fromCharCode(10) + addZeroPadding(color.toString(16).toUpperCase(),6);
         return p;
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(var_402 < 0 || var_402 > var_298.length)
         {
            var_402 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_985.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_251,onTextWindowEvent);
         var_985 = new Map();
         ...
         var_811 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_993 = 0;
         var_298 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_811);
               if(_loc11_)
               {
                  var_993 = _loc11_.breeds.length;
                  var_298 = _loc11_.colors;
                  if(var_298 == null)
                  {
                     var_298 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_993)
         {
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_811,_loc6_),getRaceLocalizationKey(var_811,_loc6_)));
            _loc6_++;
         }
         var_985.add(var_811,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_938,onApproveNameResult);
         _loc5_ = var_985.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_298)
         {
      ...
      {
         var _loc6_:* = null;
         var _loc1_:Offer = var_985.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_402 >= 0 && var_402 < var_298.length)
         {
            _loc6_ = var_298[var_402] as IPetColor;
            if(_loc6_ != null)
            {
               _loc2_ = uint(_loc6_.rgb);
            }
         }
         var _loc3_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_811,var_1464,_loc2_,AvatarScaleType.const_50);
         if(_loc3_ == null)
         {
            return;
         }
         _loc3_.initActionAppends();
         _loc3_.appendAction(AvatarAction.const_325,AvatarAction.const_678);
