package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements ICoreUpdateReceiver
   {
      ...
      private static const const_334:Number = 50;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         if(_window.height < var_1408)
         {
            _window.height = var_1408;
         }
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(_state)
         {
            ...
            case const_339:
               var_772 += param1;
               _loc2_ = Number(var_772) / Number(var_380["time_fade_in"]);
               if(var_772 > int(var_380["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_746:
               var_957 += param1;
               if(var_957 > int(var_380["time_display"]) && !var_1168)
               {
                  startFadeOut();
               }
               break;
            case const_745:
               var_771 += param1;
               _loc3_ = 1 - Number(var_771) / Number(var_380["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_771 > int(var_380["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_199.orientation == "TOP" ? int(const_334) : 0;
         var _loc4_:int = var_199.orientation == "BOTTOM" ? int(_loc2_.height - const_334) : int(_loc2_.height);
         var _loc5_:int = var_199.orientation == "LEFT" ? int(const_334) : 0;
         var _loc6_:int = var_199.orientation == "RIGHT" ? int(_loc2_.width - const_334) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_538;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_538;
            }
         }
         else if(false)
         {
      ...
      {
         if(param1.badgeId != var_208.style.iconSrc)
         {
            return;
         }
